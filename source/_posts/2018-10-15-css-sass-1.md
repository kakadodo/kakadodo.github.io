...
1. 使用命令提示字元來編譯 Sass
  早期 Sass 底層是以 Ruby 語言來撰寫，因此要使用 Sass 需要有 Ruby 環境，不過官方已經表示不再維護 Ruby Sass 了!現在比較常見的作法是使用他們推出的 Dart Sass，Dart Sass 可以在純 JavaScript 庫上運行，例如 Node.js 的 npm 管理工具就有提供 Sass 的安裝方式。如果有在使用 Webpack 或是 Gulp 的話，應該對這塊不陌生。
