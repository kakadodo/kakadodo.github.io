---
title: Sass - 前端工程師應該要會的工程化 CSS - @if、@else if、@else 和 @for、@while
keywords:
  - CSS, Sass, SCSS
categories:
  - CSS
...
要真正發揮 Sass 工程化的特性必定不能缺少的就是它提供的一系列表達式控制命令，像是 if 判斷、遍歷迴圈、自訂 function 等等，這些控制命令可以幫助撰寫 Sass 具有邏輯性且更省力!
<!-- excerpt -->
...
@if 可以直接寫在選擇器中，當然也可以另外搭配 @mixin 或 @for。
