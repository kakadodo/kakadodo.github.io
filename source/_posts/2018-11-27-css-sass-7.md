---
title: Sass - 前端工程師應該要會的工程化 CSS - @each、 @function
keywords:
  - CSS, Sass, SCSS
categories:
  - CSS
...
繼上篇介紹過的幾個指令之外，還有這次要提的 @each 和 @function。這兩個指令會稍稍特別一點，@each 需要搭配 list / map 的資料格式來運作，而 @function 則是讓你可以自訂自己的函式。
<!-- excerpt -->
...
之前提到 Sass 有兩種資料格式儲存: list 和 map。一般可以藉由預設 function 來取得單一值，例如: `nth($list，2)` 取得 $list 第二個索引的值。但如果今天是要跑全部的 list 或是全部的 map，就會需要 @each 這個指令啦!
...
## @function
使用 @function 的方式其實跟 JavaScript 沒甚麼兩樣，一樣會宣告 @function 並於需要時調用，可以帶入參數，需要使用 `@return` 才收的到產生的值。
