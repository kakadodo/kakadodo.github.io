...
在 MDN 有找到一個網站 [ECMAScript 5/6/7 compatibility tables](https://kangax.github.io/compat-table/es6/) 列出 ES6 的支援狀況，不只瀏覽器版本，連編譯工具跟 server 端都有涵蓋!實在是太佛心了!!

這邊也提供兩個介紹 ES6 語法的 gitbooks:
...
```
光看示範一可能無感，但示範二組字串要帶的變數變多，原本的組法就要不斷用引號分割。一般專案中組字串 + 變數的數量一定更多，需要利用 js 動態產生 DOM 的內容...這時就會容易進入組字串地獄 =_=

從示範三中也可以知道被反引號包起的範圍內，直接空行換行是不會被另外編譯的，可以用一般在寫 html 的方式來組標籤內容，字串模板也可巢狀應用，這樣不僅閱讀上可容易，程式碼更精簡，也不用怕一直用引號分割結果格式亂掉。

## 展開與剩餘 (spread and rest)
展開與剩餘其實是兩個截然不同的功能，只是看起來都是三個點符號 `...`，在一開始還滿容易搞混的，想說放在一起解釋比較有助於區分差異。而且看到這符號時真的會納悶 js 在開玩笑嗎? `...` 這種表達方式也太簡略了吧!!!

#### 展開 (...spread)
展開可以將物件中的值一個個的抽取出來另外存放，這種方式可以很簡單的達成物件合併、拆解二維陣列變一維，也可以做到物件的複製(淺複製)。
```js
//示範一: 合併二維陣列變成一維陣列
const concat = [...[1,2,3], 4, ...[5,6,7]];
console.log(concat); //[1,2,3,4,5,6,7]

//示範二: 複製陣列(複製後的陣列跟原陣列不是同一個參考，屬於淺複製)
...
```
使用展開符號進行物件(陣列)的複製時要特別注意子層的型別，如果抓出來的值本身是一組陣列或物件，那麼他們的參考來源還是會相同。
