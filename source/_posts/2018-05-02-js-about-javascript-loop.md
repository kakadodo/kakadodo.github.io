...
```js
  ...
  //結果為 9,7,5,3,1
```
迴圈不是 function，因此在裡面宣告的變數實際上是全域變數，使用上要小心變數被其他迴圈或函式汙染。若使用 ES6 的 let 宣告，變數會只存在於迴圈中。
...
```js
  ...
  //還是有印出num是6，即使while的條件是num<5
```

#### for in 迴圈
之前的 Object 篇有提到，for in 可以用在跑遍物件的每個屬性，當然它也可以用在陣列上，但要注意該陣列是否有另外設定過屬性，因為 for in 的特性就是會把每個屬性都抓出來，此時遍歷出來的結果就會包含陣列設定的屬性跟值。
```js
  var arr = [1,2,3];
  //arr可能因其他套件而增加了屬性的設定
  arr.test = 'test';
